"""
AI Senior Designer section — image/PDF analysis + design advice.
"""
...
SECTION = "designer"
ENTRY_LABELS = {"🎨 AI Senior Designer"}


class DesignerState(StatesGroup):
    active = State()
...
async def enter_designer(message: Message, state: FSMContext):
    ...
    await state.set_state(DesignerState.active)
    await state.update_data(lang=lang, full_name=row["full_name"], role=row["role"])
    await message.answer(
        t(lang, "designer_welcome", name=fname),
...
# ── Text message ──────────────────────────────────────────────────────────────
@router.message(DesignerState.active, F.text)
async def designer_text(message: Message, state: FSMContext):
    ...
    kb_context = await search_knowledge("designer", message.text)
...
# ── Image ─────────────────────────────────────────────────────────────────────
@router.message(DesignerState.active, F.photo)
async def designer_photo(message: Message, state: FSMContext, bot: Bot):
    data      = await state.get_data()
    lang      = data.get("lang", "uz")
    full_name = data.get("full_name", "")
    role      = data.get("role", "")
    ...
    kb_context = await search_knowledge("designer", caption or "design review")
    system     = build_system_prompt(SECTION, full_name, role, lang, kb_context)
    history    = await _get_history(tid)
...
# ── Document (PDF or any file) ────────────────────────────────────────────────
@router.message(DesignerState.active, F.document)
async def designer_document(message: Message, state: FSMContext, bot: Bot):
    data      = await state.get_data()
    lang      = data.get("lang", "uz")
    full_name = data.get("full_name", "")
    role      = data.get("role", "")
    ...
    try:
        ...
        kb_context = await search_knowledge("designer", caption or user_msg[:100])
        system     = build_system_prompt(SECTION, full_name, role, lang, kb_context)
        history    = await _get_history(tid)
